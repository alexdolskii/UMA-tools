...
def extract_channel_and_filter(
    ...
    """Headless ImageJ: extract nuclei
    channel & apply 3D filters."""
...
def main(input_file_path):
    ...
    print("  2 – Start with StarDist segmentation")
    print("  3 – Post-processing only (quantification + clustering)\n")
    mode = int(input("Enter 1, 2 or 3 ➔ ").strip())

    if mode not in [1, 2, 3]:
        ValueError("Invalid – please enter 1, 2 or 3.")

    logging.info(f"Execution mode: { {1: 'Fiji pre-processing',
                                      2: 'stardist',
                                      3: 'Fiji post-processing'}[mode]}")
    ...
    for fld in cfg["folders"]:
        ...
        if mode == 1:
            # Katya - Add check for existing directories
            outputs_exist = any(
                any((Path(f) / d).exists()
