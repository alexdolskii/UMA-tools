...
Set UMA_RUN_IMAGEJ_TESTS=1 after installing the environment and package.
These checks supplement validation on representative experimental ND2s.
"""
...
class HeadlessTests(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        try:
            ...
            # ImageJ1 workers can outlive the Fiji context.
    ...
    def setUp(self):
        ...
        tifffile.imwrite(
            self.folder / "sample.tiff",
            stack,
            imagej=True,
            resolution=(2, 2),
            metadata={"axes": "ZYX", "spacing": 1.0, "unit": "um"},
        )
        (self.folder / "._sample.tiff").write_bytes(
            b"Not an image: macOS metadata"
        )
    ...
    def test_thickness_outputs_and_metadata_filter(self):
        ...
        self.assertEqual(
            len(list(summaries[0].parent.glob("Local_Thickness_*.tif"))), 1
        )
    ...
    def test_local_thickness_api_matches_original_menu_command(self):
        ...
        np.testing.assert_allclose(
            np.array(actual.getProcessor().getPixels()),
        ...
        self.assertEqual(
            actual.getCalibration().pixelWidth,
    ...
    def test_thickness_matches_original_projection_with_calibration(self):
        ...
        IJ, image_plus, windows, table, duplicator, _ = (
            thickness.import_java_classes()
        )
        ...
        class OriginalMacroProjector:
            """Adapt the reference to former and current Python call sites."""
            ...
            def setMethod(self, method):
                if method != self.MAX_METHOD:
                    raise AssertionError(
                        "The reference requires maximum projection"
                    )
        ...
        with mock.patch.object(
            ...
            side_effect=lambda name: (
                OriginalMacroProjector
                if name == "ij.plugin.ZProjector"
        ...
        for prefix in ("Mask_", "Local_Thickness_"):
            actual_image = IJ.openImage(
                str(actual_dir / f"{prefix}{filename}.tif")
            )
            reference_image = IJ.openImage(
                str(reference_dir / f"{prefix}{filename}.tif")
            )
            try:
                self.assertIsNotNone(actual_image)
                self.assertIsNotNone(reference_image)
                self.assertEqual(
                    tuple(actual_image.getDimensions()),
                ...
                self.assertAlmostEqual(
                    reference_cal.pixelWidth, 0.75, delta=1e-6
                )
                ...
                for axis in ("pixelWidth", "pixelHeight", "pixelDepth"):
                    self.assertEqual(
                        getattr(actual_cal, axis), getattr(reference_cal, axis)
                    )
        ...
        self.assertTrue(
            np.isfinite([reference[name] for name in columns]).all()
        )
