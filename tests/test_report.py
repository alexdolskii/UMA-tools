"""Report regressions with synthetic plate data and no ImageJ."""
...
class EventLog:
    """Keep diagnostics for assertions without generating test output."""
    ...
    def event(self, level, stage, message, **kwargs):
        self.events.append(
            {
                "Timestamp_UTC": "2026-09-14T12:00:00+00:00",
                "Level": level,
                "Stage": stage,
                "Message": message,
            }
...
class ReportFixture(unittest.TestCase):
    ...
    def write_csv(path, columns, rows):
        ...
        # Preserve archived BOM and CRLF without reserializing the CSV.
    ...
    def inputs(
        ...
        names = names or (
            "клетка.v2_WellB02_field, one.nd2",
            "sample_WellB03_field.tiff",
        )
        percentages = percentages or [30.0] * len(names)
        paths = {
            role: combined / f"{self.source.name}_{filename}"
        ...
        self.write_csv(
            ...
            [
                [
                    name
                    if alignment_originals
                    else Path(name).stem + ALIGNMENT_SUFFIX,
                    "N/A",
                    "N/A",
                    60 + index,
                    "Aligned",
        ...
        manifest = json.loads(
            (combined / "run_status.json").read_text(encoding="utf-8")
        )
        manifest["copied_csvs"] = [
            {
                "analysis": assay,
                "path": f"/previous/computer/old_combined/{paths[role].name}",
    ...
    def mutate_csv(self, path, column, value, index=0):
        ...
        self.write_csv(
            path, list(rows[0]), [list(row.values()) for row in rows]
        )
    ...
    def config(self, sources, filename="inputs.json"):
        ...
        path.write_text(
            json.dumps({"folder_paths": [str(source) for source in sources]}),
            encoding="utf-8",
        )
        return path
...
class CombinedSelectionTests(ReportFixture):
    def test_latest_completed_timestamp_wins_over_mtime_and_incomplete_runs(
        self,
    ):
        older = self.make_combined("20260914_100000_999999")
        selected = self.make_combined(
            "20260914_120000_000001", status="SUCCESS_WITH_MISSING_ANALYSES"
        )
        for index, status in enumerate(
            ("RUNNING", "ERROR", "CANCELLED", "VALIDATION_FAILED"), 1
        ):
            self.make_combined(f"20260914_13000{index}_000001", status=status)
        malformed = self.make_combined("20260914_140000_000001")
        (malformed / "run_status.json").write_text(
    ...
    def test_missing_or_malformed_completion_status_is_not_completed(self):
        for index, payload in enumerate(
            (None, [], {"status": "SUCCESSFUL"}, {"status": "RUNNING"})
        ):
    ...
    def test_archive_rejects_analysis_removed_from_manifest_after_discovery(
        ...
        status["copied_csvs"] = [
            entry
            for entry in status["copied_csvs"]
            if entry["analysis"] != "Area"
        ]
...
class MergeValidationTests(ReportFixture):
    ...
    def test_full_identity_and_exact_legacy_stem_join_without_sequence_tokens(
        ...
        names = (
            "клетка.v2_WellB02_field, one.nd2",
            "sample_processed_WellB03_field.tiff",
        )
        ...
        for index, name in enumerate(names):
            ...
            self.assertAlmostEqual(
                row["Area (µm²)"], 1560.8484500537231 + index
            )
    ...
    def test_thickness_units_are_correct_without_changing_measurements(self):
        ...
        row = data["rows"][0]
        expected = {
            "Area (µm²)": 1560.8484500537231,
            "StdDev (µm)": 0.900738580354334,
            "Min (µm)": 2.2360680103302,
            "Max (µm)": 7.0,
            "Median (µm)": 5.830951690673828,
        }
    ...
    def test_strict_threshold_retains_equality_and_preserves_all_rows(self):
        ...
        data = self.merge(
            self.inputs(names=names, percentages=[19.99, 20, 20.01])
        )
        self.assertEqual(len(data["rows"]), 3)
        self.assertEqual(
            {row["Image_ID"] for row in data["retained_rows"]}, set(names[1:])
        )
        self.assertEqual(
            [row["Image_ID"] for row in data["excluded_rows"]], [names[0]]
        )
        self.assertEqual(
            {
                row["Image_ID"]
                for row in data["retained_rows"] + data["excluded_rows"]
            },
            set(names),
        )
    ...
    def test_filter_can_exclude_every_image_without_losing_original_groups(
        self,
    ):
    ...
    def test_unannotated_well_fails_with_coordinate_diagnostic(self):
        ...
        diagnostics = self.read_csv(
            self.root / "validation" / "annotation_diagnostics.csv"
        )
...
class ReportCommandTests(ReportFixture):
    def test_diagnostics_write_failure_still_removes_unverified_workbook(self):
        ...
        def fail_verification(path, *args, **kwargs):
            self.assertTrue(
                Path(path).is_file(),
                "The regression requires an existing pending workbook",
            )
        ...
        self.assertIn(
            "synthetic workbook verification failure", status["error"]
        )
    ...
    def test_unexpected_failure_writes_stage_traceback_and_error_status(self):
        ...
        self.assertIn(
            "RuntimeError",
            (outputs[0] / "traceback.txt").read_text(encoding="utf-8"),
        )
        self.assertIn(
            "synthetic dependency failure",
            (outputs[0] / "run.log").read_text(encoding="utf-8"),
        )
    ...
    def test_cli_continues_after_latest_failure_and_exports_report(
        ...
        names = (
            ...
            "equal_WellC02.nd2",
            "low_WellD02.nd2",
            "kept_WellD03_first.nd2",
            "kept_WellD03_second.nd2",
        )
        groups = {
            "B02": "Empty group",
            "C02": "Singleton group",
        ...
        self.assertTrue(
            executable.is_file(),
            "The wheel must install the uma_report console command",
        )
        result = subprocess.run(
            [str(executable), "-i", str(config), "--fn-threshold", "20"],
            cwd=self.root,
            capture_output=True,
            text=True,
            timeout=180,
        ...
        for source, original in original_bytes.items():
            ...
            self.assertEqual(
                (output / "Inputs" / source.name).read_bytes(), original
            )
        self.assertEqual(
            (output / "Inputs" / "input_paths.json").read_bytes(),
            config.read_bytes(),
        )
        self.assertEqual(
            (output / "Inputs" / "collector_run_status.json").read_bytes(),
        ...
        for plot in plots:
            ...
            self.assertEqual(
                plot["group_order"],
                ["Empty group", "Singleton group", "Mixed group"],
            )
        ...
        try:
            ...
            self.assertEqual(
                {row["Image_ID"] for row in exported["Merged Data"]},
                set(names),
            )
            self.assertEqual(
                {row["Image_ID"] for row in exported["Filtered Data"]},
                set(names[1:2] + names[3:]),
            )
            self.assertEqual(
                {row["Image_ID"] for row in exported["Excluded Data"]},
                {names[0], names[2]},
            )
            equal_row = next(
                row
                for row in exported["Filtered Data"]
                if row["Image_ID"] == names[1]
            )
    ...
    def test_failure_only_cli_leaves_diagnostics_and_no_workbook(self):
        ...
        self.assertNotEqual(
            json.loads((outputs[0] / "run_status.json").read_text())["status"],
            "SUCCESS",
        )
        self.assertEqual(list(outputs[0].glob("*.xlsx")), [])
