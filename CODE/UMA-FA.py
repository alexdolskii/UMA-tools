...
for folder_idx, input_folder in enumerate(folder_paths, start=1):
    ...
    # Create a new processed_images folder with date and time inside each input folder
    ...
    print(f"Processed images will be saved in: {processed_folder}")
    ...
    timestamp_analysis = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_folder = os.path.join(analysis_folder, f'Results_{timestamp_analysis}')
    ...
    for file_name in file_list:
        ...
        try:
            ...
            output_file_name = f'{os.path.splitext(file_name)[0]}_processed_{timestamp_analysis}.xlsx'
    ...
    summary_file_path = os.path.join(results_folder, f'Alignment_Summary_{timestamp_analysis}.xlsx')
    summary_df.to_excel(summary_file_path, index=False)
    print(f"\nSummary data saved to: {summary_file_path}")

    print(f"\nProcessing completed for folder {input_folder}. All results are saved in folder: {processed_folder}")
